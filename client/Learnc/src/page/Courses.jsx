...
import { Link } from 'react-router-dom';// Adjust this path according to your project structure
...
const Courses = () => {
  ...
  if (error) {
    return <p>Error: {error.detail || "Something went wrong!"}</p>; // Handle the error object properly
  }
        ...
        {courses.length > 0 ? (
          courses.map(course => (
            <div key={course.id} className="bg-white shadow-lg rounded-lg overflow-hidden">
              <img
                src={course.profile_picture}// Adjust according to your data structure
                alt={course.title}
                height={200}
                width={300}
